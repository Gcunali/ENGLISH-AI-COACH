...
const ANCHORS: &[(u32, u32)] = &[
    (0, 5), (1, 10), (10, 45), (20, 90), (30, 145), (40, 205),
    (50, 265), (60, 325), (70, 380), (80, 425), (90, 465),
    (95, 480), (100, 495),
];
...
pub fn estimate(raw_correct: u32) -> Result<ListeningEstimate, String> {
    ...
    let uncertainty = if raw_correct < 20 { 30 } else if raw_correct < 80 { 25 } else { 20 };
...
fn interpolate(raw: u32) -> u32 {
    if raw == 100 { return 495; }
    let (left, right) = ANCHORS.windows(2)
...
fn round_five(value: u32) -> u32 { ((value + 2) / 5) * 5 }
...
mod tests {
    ...
    fn anchors_are_stable_and_in_five_point_steps() {
        for (raw, expected) in [(0,5),(1,10),(10,45),(20,90),(50,265),(75,405),(90,465),(100,495)] {
    ...
    fn all_raw_scores_are_monotonic() {
        let values=(0..=100).map(|raw| estimate(raw).unwrap().estimated_score).collect::<Vec<_>>();
        assert!(values.windows(2).all(|pair| pair[0] <= pair[1]));
    }
    #[test]
    fn rejects_incomplete_or_impossible_raw_domain() { assert!(estimate(101).is_err()); }
}
