...
export function HistoryPage() {
  ...
  return <>
    <header className="mb-7"><p className="muted text-xs uppercase tracking-[.18em] mb-2">Local lesson archive</p><h1 className="m-0 text-2xl md:text-3xl">History</h1><p className="muted text-sm">Persisted lessons, newest first. Opening this page never calls the analyzer.</p></header>
    ...
    {query.data && query.data.items.length === 0 && <EmptyState title="No lessons found" message={filter === 'all' ? 'Your lessons will appear here after you complete your first conversation.' : 'No persisted lessons match this filter.'} />}
...
function LessonHistoryCard({ lesson }: { lesson: LessonHistoryItem }) {
  ...
  return <article className="glass rounded-2xl p-5"><div className="flex flex-wrap items-center justify-between gap-4"><div className="min-w-0"><div className="flex items-center gap-2"><StatusIcon size={16} className={lesson.status === 'completed' ? 'text-[var(--accent)]' : 'text-amber-300'} /><h2 className="m-0 truncate text-base">{lessonTitle(lesson.topic)}</h2></div><p className="muted mb-0 mt-2 text-xs">{formatLocalDate(lesson.startedAt)} · {formatDuration(lesson.durationSeconds)} · {humanize(lesson.mode)}</p><div className="mt-3 flex flex-wrap gap-2 text-[11px]"><Badge label={`Lesson: ${humanize(lesson.status)}`} /><Badge label={`${lesson.studentTurnCount} student turns`} /><Badge label={`${lesson.correctionCount} corrections`} /><Badge label={`Analysis: ${lesson.analysisStatus ? humanize(lesson.analysisStatus) : 'Not analyzed'}`} /></div></div><div className="flex items-center gap-4">{lesson.overallScore !== null && <div className="text-center"><strong className="text-2xl text-[var(--accent)]">{lesson.overallScore}</strong><div className="muted text-[10px]">Overall</div></div>}<Link to={`/history/${lesson.id}`} className="rounded-full border border-white/15 px-4 py-2 text-sm text-white no-underline">View details</Link></div></div></article>
}
