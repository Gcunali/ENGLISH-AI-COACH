...
export function PracticeConsistency({ compact = false }: { compact?: boolean }) {
  ...
  return <section className="glass rounded-[24px] p-5 md:p-6" aria-label="Practice Consistency">
    ...
    <div className="mt-4 h-2 overflow-hidden rounded-full bg-white/[.07]" aria-label={`${value.weeklyGoal.progressPercent}% of weekly goal`}><div className="h-full rounded-full bg-[var(--accent)]" style={{ width: `${value.weeklyGoal.progressPercent}%` }} /></div>
