...
impl LearningMemoryRepository {
        ...
        connection
            .query_row(
                "SELECT COUNT(*),
                        ...
                        (SELECT COUNT(*) FROM (
                           SELECT 'standard:' || lesson_id AS source_lesson FROM lesson_vocabulary
                           UNION SELECT 'guided:' || lesson_id FROM guided_session_vocabulary
                         ))
    ...
    pub fn list_vocabulary(
        ...
        let sql = format!(
            "SELECT v.id, v.display_text, v.meaning, v.status, v.first_seen_at, v.last_seen_at,
                    ...
                    (SELECT example FROM (
                       SELECT lv.example,l.started_at AS seen_at,lv.id AS source_id
                       FROM lesson_vocabulary lv JOIN lesson l ON l.id=lv.lesson_id
                       WHERE lv.vocabulary_item_id=v.id
                       UNION ALL
                       SELECT gv.example,s.completed_at,gv.id
                       FROM guided_session_vocabulary gv JOIN interactive_lesson_session s ON s.id=gv.session_id
                       WHERE gv.vocabulary_item_id=v.id
                     ) ORDER BY seen_at DESC,source_id DESC LIMIT 1)
    ...
    pub fn get_vocabulary_item(
        ...
        let occurrences = connection
            .prepare(
                "SELECT lesson_id,source_type,lesson_date,example,occurrence_count FROM (
                   SELECT lv.lesson_id,'standard' AS source_type,l.started_at AS lesson_date,
                          lv.example,lv.occurrence_count,lv.id AS source_id
                   FROM lesson_vocabulary lv JOIN lesson l ON l.id=lv.lesson_id
                   WHERE lv.vocabulary_item_id=?1
                   UNION ALL
                   SELECT gv.lesson_id,'guided',s.completed_at,gv.example,gv.occurrence_count,gv.id
                   FROM guided_session_vocabulary gv JOIN interactive_lesson_session s ON s.id=gv.session_id
                   WHERE gv.vocabulary_item_id=?1
                 ) ORDER BY lesson_date DESC,source_id DESC",
            )
    ...
    pub fn update_vocabulary_status(
        ...
        let transaction = connection
            .transaction()
            .map_err(|error| format!("Could not start vocabulary status transaction: {error}"))?;
        update_vocabulary_status_in_transaction(&transaction, vocabulary_id, status)?;
        transaction
            .commit()
            .map_err(|error| format!("Could not commit vocabulary status transaction: {error}"))?;
    ...
    pub fn get_recurring_mistake(
        ...
        let occurrences = connection
            .prepare(
                "SELECT lesson_id,source_type,lesson_date,original,corrected,explanation FROM (
                   SELECT o.lesson_id,'standard' AS source_type,l.started_at AS lesson_date,
                          o.original,o.corrected,o.explanation,o.source_index,o.id AS source_id
                   FROM recurring_mistake_occurrence o JOIN lesson l ON l.id=o.lesson_id
                   WHERE o.recurring_mistake_id=?1
                   UNION ALL
                   SELECT o.lesson_id,'guided',s.completed_at,o.original,o.corrected,o.explanation,
                          o.source_index,o.id
                   FROM guided_recurring_mistake_occurrence o
                   JOIN interactive_lesson_session s ON s.id=o.session_id
                   WHERE o.recurring_mistake_id=?1
                 ) ORDER BY lesson_date DESC,source_index,source_id",
            )
...
pub(crate) fn update_vocabulary_status_in_transaction(
        ...
        .execute(
            &format!(
                "UPDATE vocabulary_item SET
                   lesson_count = (SELECT COUNT(*) FROM (
                     SELECT 'standard:'||lesson_id FROM lesson_vocabulary WHERE vocabulary_item_id=?1
                     UNION SELECT 'guided:'||lesson_id FROM guided_session_vocabulary WHERE vocabulary_item_id=?1
                   )),
                   occurrence_count = (SELECT COALESCE(SUM(value),0) FROM (
                     SELECT occurrence_count AS value FROM lesson_vocabulary WHERE vocabulary_item_id=?1
                     UNION ALL SELECT occurrence_count FROM guided_session_vocabulary WHERE vocabulary_item_id=?1
                   )),
                   first_seen_at = (SELECT MIN(seen_at) FROM (
                     SELECT l.started_at AS seen_at FROM lesson_vocabulary lv JOIN lesson l ON l.id=lv.lesson_id WHERE lv.vocabulary_item_id=?1
                     UNION ALL SELECT s.completed_at FROM guided_session_vocabulary gv JOIN interactive_lesson_session s ON s.id=gv.session_id WHERE gv.vocabulary_item_id=?1
                   )),
                   last_seen_at = (SELECT MAX(seen_at) FROM (
                     SELECT l.started_at AS seen_at FROM lesson_vocabulary lv JOIN lesson l ON l.id=lv.lesson_id WHERE lv.vocabulary_item_id=?1
                     UNION ALL SELECT s.completed_at FROM guided_session_vocabulary gv JOIN interactive_lesson_session s ON s.id=gv.session_id WHERE gv.vocabulary_item_id=?1
                   )),
                   updated_at = {NOW_SQL}
        ...
        .execute(
            &format!(
                "UPDATE recurring_mistake SET
                   lesson_count = (SELECT COUNT(*) FROM (
                     SELECT 'standard:'||lesson_id FROM recurring_mistake_occurrence WHERE recurring_mistake_id=?1
                     UNION SELECT 'guided:'||lesson_id FROM guided_recurring_mistake_occurrence WHERE recurring_mistake_id=?1
                   )),
                   occurrence_count = ((SELECT COUNT(*) FROM recurring_mistake_occurrence WHERE recurring_mistake_id=?1)+(SELECT COUNT(*) FROM guided_recurring_mistake_occurrence WHERE recurring_mistake_id=?1)),
                   first_seen_at = (SELECT MIN(seen_at) FROM (
                     ...
                     UNION ALL SELECT s.completed_at FROM guided_recurring_mistake_occurrence o JOIN interactive_lesson_session s ON s.id=o.session_id WHERE o.recurring_mistake_id=?1
                   )),
                   last_seen_at = (SELECT MAX(seen_at) FROM (
                     SELECT l.started_at AS seen_at FROM recurring_mistake_occurrence o JOIN lesson l ON l.id=o.lesson_id WHERE o.recurring_mistake_id=?1
                     UNION ALL SELECT s.completed_at FROM guided_recurring_mistake_occurrence o JOIN interactive_lesson_session s ON s.id=o.session_id WHERE o.recurring_mistake_id=?1
