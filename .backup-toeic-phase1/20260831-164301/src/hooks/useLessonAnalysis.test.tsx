...
describe('useLessonAnalysis', () => {
  ...
  it('does not analyze a completed lesson restored only for historical display', async () => {
    useConversationStore.getState().setSummary(summary, false)
    mocks.getLessonAnalysis.mockResolvedValue(null)
    renderHook(() => useLessonAnalysis())
    await waitFor(() => expect(mocks.getLessonAnalysis).toHaveBeenCalledWith(lesson.id))
    expect(mocks.analyzeLesson).not.toHaveBeenCalled()
  })
