use crate::{
    ...
    student_learning_summary_repository::StudentLearningSummaryRepository,
};
use reqwest::Client;
...
impl LessonAnalyzer {
    ...
    fn sync_memory_without_blocking(&self, analysis: &LessonAnalysis) {
        ...
        if let Err(error) = self.summaries.refresh_summary() {
            log::error!(
                "Lesson analysis {} completed, but student learning summary refresh failed: {}",
                analysis.id,
                error
            );
        }
...
mod tests {
    ...
    fn completed_analysis_syncs_memory_without_another_model_request() {
        ...
        let summaries = StudentLearningSummaryRepository::new(path.clone());
        let analyzer =
            LessonAnalyzer::new(lessons, analyses, memory.clone(), summaries.clone()).unwrap();
    ...
    fn physical_analyzes_latest_completed_lesson() {
        tauri::async_runtime::block_on(async {
            ...
            let memory = LearningMemoryRepository::new(database_path.clone());
            let summaries = StudentLearningSummaryRepository::new(database_path);
            let analyzer = LessonAnalyzer::new(lessons, analyses, memory, summaries)
                .expect("local analyzer client");
