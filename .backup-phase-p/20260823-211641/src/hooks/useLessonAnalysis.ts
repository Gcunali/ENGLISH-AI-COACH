...
export function useLessonAnalysis() {
  ...
  const autoEligible = useConversationStore((store) => store.analysisAutoEligible)
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!disposed) {
          useConversationStore.getState().setAnalysis(completed)
          notifyLearningDataChanged()
        }
      } catch (requestError) {
        if (disposed) return
