...
impl ToeicRepository {
    ...
    fn history_query(
        ...
        let connection = database::open(&self.database)?;
        let mut sql="SELECT s.id,s.form_id,s.status,s.created_at,s.updated_at,s.completed_at,COUNT(a.id),COALESCE(SUM(a.is_correct),0),s.part FROM toeic_session s LEFT JOIN toeic_answer a ON a.session_id=s.id".to_owned();
        if status.is_some() {
        ...
        let mapper = |r: &rusqlite::Row<'_>| -> rusqlite::Result<ToeicHistoryEntryDto> {
            ...
            let total = if r.get::<_, String>(8)? == "part2_question_response" {
                25
            } else {
                FORM_LENGTH as u32
            };
