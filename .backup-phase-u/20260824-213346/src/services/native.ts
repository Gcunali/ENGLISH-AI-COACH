...
import type { BackupStatus, BackupSummary, BackupValidation, RestoreScheduled, SystemDiagnostics, SystemEvent, WelcomeState } from '../types'
import type { ExerciseResponse, GuidedAudio, GuidedLessonDetail, GuidedLessonOverview, GuidedLessonSession, GuidedLessonSummary } from '../types'
...
export async function getWelcomeState(): Promise<WelcomeState> {
  if (!isTauri()) return { shouldShow: false, hasSeen: true, existingUser: false }
  return invoke<WelcomeState>('get_welcome_state')
}

export async function setWelcomeSeen(seen = true): Promise<WelcomeState> {
  if (!isTauri()) return { shouldShow: !seen, hasSeen: seen, existingUser: false }
...
export async function getGuidedLessonOverview():Promise<GuidedLessonOverview>{if(!isTauri())return{publishedLessonCount:0,activeSession:null,capabilities:[]};return invoke<GuidedLessonOverview>('get_guided_lesson_overview')}
export async function listGuidedLessons():Promise<GuidedLessonSummary[]>{if(!isTauri())return[];return invoke<GuidedLessonSummary[]>('list_guided_lessons')}
export async function getGuidedLesson(lessonId:string):Promise<GuidedLessonDetail|null>{if(!isTauri())return null;return invoke<GuidedLessonDetail|null>('get_guided_lesson',{lessonId})}
export async function getActiveGuidedLessonSession():Promise<GuidedLessonSession|null>{if(!isTauri())return null;return invoke<GuidedLessonSession|null>('get_active_guided_lesson_session')}
export async function startGuidedLesson(lessonId:string,startOver=false):Promise<GuidedLessonSession>{if(!isTauri())throw new Error('Guided Lessons require the desktop runtime.');return invoke<GuidedLessonSession>('start_guided_lesson',{request:{lessonId,startOver}})}
export async function resumeGuidedLesson(sessionId:string):Promise<GuidedLessonSession>{if(!isTauri())throw new Error('Guided Lessons require the desktop runtime.');return invoke<GuidedLessonSession>('resume_guided_lesson',{sessionId})}
export async function getGuidedLessonSession(sessionId:string):Promise<GuidedLessonSession|null>{if(!isTauri())return null;return invoke<GuidedLessonSession|null>('get_guided_lesson_session',{sessionId})}
export async function completeGuidedLessonStage(sessionId:string,stageId:string):Promise<GuidedLessonSession>{if(!isTauri())throw new Error('Guided Lessons require the desktop runtime.');return invoke<GuidedLessonSession>('complete_guided_lesson_stage',{request:{sessionId,stageId}})}
export async function skipGuidedLessonStage(sessionId:string,stageId:string):Promise<GuidedLessonSession>{if(!isTauri())throw new Error('Guided Lessons require the desktop runtime.');return invoke<GuidedLessonSession>('skip_guided_lesson_stage',{request:{sessionId,stageId}})}
export async function abandonGuidedLesson(sessionId:string):Promise<GuidedLessonSession>{if(!isTauri())throw new Error('Guided Lessons require the desktop runtime.');return invoke<GuidedLessonSession>('abandon_guided_lesson',{sessionId})}
export async function listRecentGuidedLessonSessions(limit=10):Promise<GuidedLessonSession[]>{if(!isTauri())return[];return invoke<GuidedLessonSession[]>('list_recent_guided_lesson_sessions',{limit})}
const guidedItemRequest=(sessionId:string,stageId:string,itemId:string)=>({sessionId,stageId,itemId})
export async function prepareGuidedLessonAudio(sessionId:string,stageId:string,itemId:string):Promise<GuidedAudio>{if(!isTauri())throw new Error('Guided audio requires the desktop runtime.');return invoke<GuidedAudio>('prepare_guided_lesson_audio',{request:guidedItemRequest(sessionId,stageId,itemId)})}
export async function completeGuidedLessonAudio(playbackId:string,sessionId:string,stageId:string,itemId:string):Promise<GuidedLessonSession>{return invoke<GuidedLessonSession>('complete_guided_lesson_audio',{playbackId,request:guidedItemRequest(sessionId,stageId,itemId)})}
export async function cancelGuidedLessonAudio(playbackId:string):Promise<boolean>{if(!isTauri())return false;return invoke<boolean>('cancel_guided_lesson_audio',{playbackId})}
export async function submitGuidedLessonPronunciation(sessionId:string,stageId:string,itemId:string,audioBase64:string):Promise<GuidedLessonSession>{return invoke<GuidedLessonSession>('submit_guided_lesson_pronunciation',{request:{...guidedItemRequest(sessionId,stageId,itemId),audioBase64}})}
export async function cancelGuidedLessonPronunciation():Promise<boolean>{if(!isTauri())return false;return invoke<boolean>('cancel_guided_lesson_pronunciation')}
export async function selectGuidedLessonPronunciationAttempt(sessionId:string,stageId:string,itemId:string,attemptId:string):Promise<GuidedLessonSession>{return invoke<GuidedLessonSession>('select_guided_lesson_pronunciation_attempt',{request:{...guidedItemRequest(sessionId,stageId,itemId),attemptId}})}
export async function submitGuidedLessonExerciseAttempt(sessionId:string,stageId:string,exerciseId:string,submissionId:string,response:ExerciseResponse):Promise<GuidedLessonSession>{if(!isTauri())throw new Error('Guided Exercise requires the desktop runtime.');return invoke<GuidedLessonSession>('submit_guided_lesson_exercise_attempt',{request:{sessionId,stageId,exerciseId,submissionId,response}})}
export async function selectGuidedLessonExerciseAttempt(sessionId:string,stageId:string,exerciseId:string,attemptId:string):Promise<GuidedLessonSession>{if(!isTauri())throw new Error('Guided Exercise requires the desktop runtime.');return invoke<GuidedLessonSession>('select_guided_lesson_exercise_attempt',{request:{sessionId,stageId,exerciseId,attemptId}})}
