...
export function SettingsPage() {
  ...
  return <>
    <header className="mb-7">
      <p className="muted mb-2 text-xs uppercase tracking-[.18em]">Local preferences</p>
      <h1 className="m-0 text-2xl md:text-3xl">Settings</h1>
      <p className="muted text-sm">Control voice performance and how persisted learning history supports future lessons.</p>
    </header>

    <section className="glass rounded-[28px] p-5 md:p-7" aria-labelledby="voice-performance-title">
      <p className="muted mb-3 text-xs uppercase tracking-[.18em]">Voice Performance</p>
      <div className="flex flex-col items-start justify-between gap-5 xl:flex-row">
        <div className="min-w-0 flex flex-1 gap-3">
          <Gauge className="mt-1 shrink-0 text-[var(--accent)]" size={21} />
          <div><h2 id="voice-performance-title" className="m-0 text-lg">Faster voice responses</h2><p className="muted mb-0 mt-2 text-sm">Starts speaking while the local AI is still finishing its response.</p></div>
        </div>
        <button type="button" role="switch" aria-label="Faster voice responses" aria-checked={streamingEnabled ?? false} disabled={streamingEnabled === null || savingStreaming} onClick={toggleStreaming} className={`min-w-24 shrink-0 self-start rounded-full border px-4 py-2 text-sm xl:self-auto ${streamingEnabled ? 'border-[var(--accent)] bg-[var(--accent)]/10 text-[var(--accent)]' : 'border-white/10 bg-white/[.03] text-white'} disabled:opacity-50`}>{streamingEnabled ? 'On' : 'Off'}</button>
      </div>
    ...
    <section className="glass mt-5 rounded-[28px] p-5 md:p-7">
      <div className="flex flex-col items-start justify-between gap-5 xl:flex-row">
        ...
        <button
          type="button"
          role="switch"
          aria-label="Use learning memory in lessons"
          aria-checked={enabled ?? false}
          disabled={enabled === null || saving}
          onClick={toggleMemory}
          className={`min-w-24 shrink-0 self-start rounded-full border px-4 py-2 text-sm xl:self-auto ${enabled ? 'border-[var(--accent)] bg-[var(--accent)]/10 text-[var(--accent)]' : 'border-white/10 bg-white/[.03] text-white'} disabled:opacity-50`}
