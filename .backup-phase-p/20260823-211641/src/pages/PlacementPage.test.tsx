...
describe('PlacementPage', () => {
  ...
  it('offers resume and start over for an active attempt', () => { mocks.useOverview.mockReturnValue({data:{...overview,activeAttempt:attempt},loading:false,error:null,reload:vi.fn()}); render(<MemoryRouter><PlacementPage /></MemoryRouter>); expect(screen.getByRole('button',{name:'Resume placement'})).toBeInTheDocument(); expect(screen.getByRole('button',{name:'Start over'})).toBeInTheDocument() })
