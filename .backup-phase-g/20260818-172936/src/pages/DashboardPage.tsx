...
import { useDashboardData } from '../hooks/useLearningData'
    ...
    <div className="content-grid grid grid-cols-[minmax(0,1fr)_330px] gap-5">
      <section className="glass rounded-[28px] min-h-[560px] p-5 md:p-8 flex flex-col" aria-label="Conversation workspace">
        ...
        <div className="flex-1 flex flex-col items-center justify-center py-10">
          ...
          {!active ? <button onClick={() => void voice.startLesson()} disabled={!probe.offlineReady || voice.engineState === 'starting'} className="mt-5 bg-[var(--accent)] disabled:opacity-50 text-[#081006] font-semibold rounded-full px-6 py-3 flex items-center gap-2"><Play size={17} fill="currentColor" /> Start Lesson</button>
            : <button onClick={() => void voice.endLesson()} className="mt-5 rounded-full border border-red-300/20 px-5 py-3 text-red-200 flex items-center gap-2"><Square size={14} fill="currentColor" /> End Lesson</button>}
