    ...
    async fn run(&self, lesson_id: &str, retry: bool) -> Result<LessonAnalysis, String> {
        ...
        if let Some(existing) = self.analyses.get_by_lesson(lesson_id)? {
            match (existing.status, retry) {
                (LessonAnalysisStatus::Completed, _) => return Ok(existing),
...
mod tests {
    ...
    use crate::lesson_analysis::{
        ...
        PedagogicalLesson, PedagogicalMessage,
    };
    ...
    fn physical_analyzes_latest_completed_lesson() {
        tauri::async_runtime::block_on(async {
            ...
            let analyses = LessonAnalysisRepository::new(database_path);
            let analyzer = LessonAnalyzer::new(lessons, analyses).expect("local analyzer client");
