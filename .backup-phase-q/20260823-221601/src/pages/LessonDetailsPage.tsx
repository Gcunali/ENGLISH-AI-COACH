...
const GOAL_LABELS: Record<string, string> = { general_fluency: 'General Fluency', everyday_conversation: 'Everyday Conversation', travel_english: 'Travel English', professional_english: 'Professional English', job_interview: 'Job Interview', academic_english: 'Academic English', grammar_accuracy: 'Grammar Accuracy', vocabulary_growth: 'Vocabulary Growth', speaking_confidence: 'Speaking Confidence', exam_preparation: 'Exam Preparation' }
...
export function LessonDetailsPage() {
  ...
  const { lesson, messages, analysis, configuration, studentProfileSnapshot } = query.data
  ...
  return <>
    ...
    <section className="glass mt-5 rounded-[28px] p-5 md:p-7" aria-label="Profile at lesson start"><h2 className="mt-0 text-lg">Profile at lesson start</h2>{!studentProfileSnapshot ? <p className="muted mb-0 text-sm">Student profile context was not recorded for this lesson.</p> : !studentProfileSnapshot.contextEnabled ? <p className="muted mb-0 text-sm">Student Profile was disabled when this lesson started. No estimated level or goals were supplied to the teacher.</p> : <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Detail label="Estimated level used" value={studentProfileSnapshot.estimatedCefrLevel ?? 'Not assessed'} /><Detail label="Confidence" value={studentProfileSnapshot.placementConfidence ? humanize(studentProfileSnapshot.placementConfidence) : 'Not available'} /><Detail label="Target" value={studentProfileSnapshot.targetCefrLevel ?? 'No target'} /><Detail label="Goals" value={studentProfileSnapshot.learningGoals.length ? studentProfileSnapshot.learningGoals.map((goal) => GOAL_LABELS[goal]).join(', ') : 'No goals'} /><Detail label="Lesson difficulty" value={humanize(studentProfileSnapshot.lessonDifficulty)} /></div>}</section>
