...
export function VocabularyPage() {
  ...
  return <PageShell width="standard">
    <PageHeader eyebrow="Derived from completed analyses" title="Vocabulary" description="Words and phrases detected in your analyzed lessons, with their original examples." actions={<Link to="/review?mode=vocabulary" className="button-secondary">Review Vocabulary</Link>} />
    ...
    <section className="glass rounded-[28px] p-5 md:p-7">
      <div className="mb-4"><InlineNotice tone="info"><strong>New</strong> has not been reviewed, <strong>Learning</strong> is in active practice, and <strong>Known</strong> is a status you explicitly choose.</InlineNotice></div>
      ...
      {query.data?.items.length === 0 && <div className="mt-5"><EmptyState title={search || filter !== 'all' ? 'No matching vocabulary' : 'No vocabulary yet'} message={search || filter !== 'all' ? 'Try another search or status filter.' : 'Vocabulary appears here after a completed lesson analysis identifies useful words or phrases.'} action={!search && filter === 'all' ? <Link to="/lesson/new" className="button-primary">Start a Lesson</Link> : undefined} /></div>}
