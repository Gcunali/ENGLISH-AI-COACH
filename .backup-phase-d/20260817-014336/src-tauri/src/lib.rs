...
use voice_engine::{VoiceEngineManager, VoiceEngineStatus};
...
async fn start_voice_engine(
    ...
    state.voice_engine.start(app, &state.local_ai)
}
    ...
    tauri::Builder::default()
        ...
        .on_window_event(|window, event| {
            if matches!(event, tauri::WindowEvent::Destroyed) {
                window.state::<AppState>().voice_engine.shutdown();
            }
        })
