...
export type PronunciationStatus = 'completed' | 'content_mismatch' | 'insufficient_audio' | 'alignment_failed' | 'engine_unavailable' | 'cancelled' | 'failed'
export type PronunciationConfidence = 'low' | 'medium' | 'high'
export interface PronunciationPhoneResult { phone:string; score:number; startMs:number; endMs:number; frameCount:number; closestAlternative:string|null; hint:string|null }
export interface PronunciationWordResult { index:number; word:string; score:number; startMs:number; endMs:number; expectedPhones:string[]; phoneResults:PronunciationPhoneResult[] }
export interface PronunciationAttempt { id:string; status:PronunciationStatus; sourceType:'custom'|'vocabulary'|'diagnostic'|'interactive_lesson'; sourceId:string|null; targetText:string; locale:'en-US'; overallScore:number|null; confidence:PronunciationConfidence|null; contentMatchScore:number|null; alignmentCoverage:number|null; audioDurationMs:number|null; createdAt:string; completedAt:string|null; words:PronunciationWordResult[] }
export interface PronunciationEngineStatus { installed:boolean; available:boolean; ready:boolean; engineVersion:number; scoreVersion:number; resultSchemaVersion:number; modelId:string; modelRevision:string; phonemizerReady:boolean; loadMs:number|null; lastError:string|null }
...
export interface DiagnosticComponent { status:DiagnosticStatus; version:string|null; message:string; technicalCode:string|null; advancedDetails:Record<string,unknown> }
export interface SystemDiagnostics { reportVersion:number; generatedAt:string; appVersion:string; platform:string; database:DiagnosticComponent; ollama:DiagnosticComponent; whisper:DiagnosticComponent; piper:DiagnosticComponent; voiceBridge:DiagnosticComponent; voiceStreaming:DiagnosticComponent; pronunciation:DiagnosticComponent; settings:DiagnosticComponent; overallStatus:string; conversationReady:boolean; pronunciationReady:boolean; databaseReady:boolean }
export interface BackupSummary { backupId:string; createdAt:string; path:string; databaseBytes:number; databaseSha256:string; schemaVersion:number; valid:boolean }
export interface BackupValidation { valid:boolean; backupId:string; schemaVersion:number|null; integrity:string; foreignKeyViolations:number; message:string }
export interface RestoreResultRecord { status:'completed'|'failed'; backupId:string; message:string; occurredAt:string }
export interface BackupStatus { operation:'idle'|'creating'|'validating'|'creating_safety_backup'|'restoring'|'reopening'|'completed'|'failed'; error:string|null; backupDirectory:string; lastBackup:BackupSummary|null; restoreAllowed:boolean; restoreBlockReason:string|null; pendingRestart:boolean; lastRestore:RestoreResultRecord|null }
export interface RestoreScheduled { backupId:string; safetyBackupId:string; restartRequired:boolean; message:string }
export interface SystemEvent { id:string; severity:'warning'|'error'|'recovery'; component:string; eventCode:string; details:Record<string,unknown>|null; occurredAt:string }
export interface WelcomeState { shouldShow:boolean; hasSeen:boolean; existingUser:boolean }

export type InteractiveStageType = 'theory'|'visual_vocabulary'|'listening'|'repeat'|'speaking_check'|'exercise'|'guided_conversation'|'analysis'
export type InteractiveSessionStatus = 'in_progress'|'completed'|'abandoned'|'failed'
export type InteractiveStageStatus = 'pending'|'active'|'completed'|'skipped'
export interface GuidedStageCapability { stageType:InteractiveStageType; stageSchemaVersion:number; runtimeAvailable:boolean }
export interface GuidedLessonSummary { lessonId:string; contentVersion:number; title:string; description:string; cefrBand:CefrBand; estimatedMinutes:number; objectives:string[]; tags:string[]; stageCount:number; startable:boolean; unavailableReasons:string[] }
export interface GuidedStageOverview { stageId:string; stageType:InteractiveStageType; title:string; required:boolean; available:boolean }
export interface GuidedLessonDetail extends GuidedLessonSummary { stageOverview:GuidedStageOverview[] }
export interface GuidedSessionStage { stageId:string; sequenceIndex:number; stageType:InteractiveStageType; title:string; required:boolean; status:InteractiveStageStatus; attemptCount:number }
export interface TheoryBlock { type:'paragraph'|'bullet_list'|'example'|'callout'; text?:string|null; items?:string[]|null; english?:string|null; explanation?:string|null; title?:string|null }
export interface VisualVocabularyItem { itemId:string; term:string; meaning:string; example:string; imageAssetId:string|null }
export interface GuidedPronunciationAttempt { id:string;attemptIndex:number;status:PronunciationStatus|'analyzing';selected:boolean;result:PronunciationAttempt|null;createdAt:string;completedAt:string|null }
export interface GuidedListeningSegment { segmentId:string;text:string;hasBundledAudio:boolean;completedPlaybackCount:number }
export interface GuidedRepeatTarget { targetId:string;text:string;hint:string|null;hasBundledAudio:boolean;completedReferencePlaybackCount:number;selectedAttemptId:string|null;attempts:GuidedPronunciationAttempt[] }
export interface GuidedSpeakingTarget { targetId:string;instruction:string;targetText:string;hint:string|null;selectedAttemptId:string|null;attempts:GuidedPronunciationAttempt[] }
export type ExerciseType='single_choice'|'multiple_select'|'fill_blank'|'word_order'|'matching'|'short_answer_exact'
export interface ExerciseOption { optionId:string;text:string }
export interface ExerciseToken { tokenId:string;text:string }
export interface ExerciseMatchingItem { itemId:string;text:string }
export interface ExerciseMatchingPair { leftId:string;rightId:string }
export type ExerciseExpectedAnswer={kind:'single_choice';option:ExerciseOption}|{kind:'multiple_select';options:ExerciseOption[]}|{kind:'fill_blank';answer:string}|{kind:'word_order';tokens:ExerciseToken[]}|{kind:'matching';pairs:{left:ExerciseMatchingItem;right:ExerciseMatchingItem}[]}|{kind:'short_answer_exact';answer:string}
export interface GuidedExerciseAttempt { attemptId:string;attemptIndex:number;correct:boolean;selected:boolean;feedback:string;explanation:string|null;expectedAnswer:ExerciseExpectedAnswer;normalizationVersion:number;submittedAt:string;selectedAt:string|null }
interface GuidedExerciseItemBase { exerciseId:string;prompt:string;instructions:string|null;hint:string|null;attempts:GuidedExerciseAttempt[];selectedAttemptId:string|null }
export type GuidedExerciseItem=
  | GuidedExerciseItemBase&{exerciseType:'single_choice'|'multiple_select';payload:{options:ExerciseOption[]}}
  | GuidedExerciseItemBase&{exerciseType:'fill_blank';payload:{prefix:string;suffix:string}}
  | GuidedExerciseItemBase&{exerciseType:'word_order';payload:{tokens:ExerciseToken[]}}
  | GuidedExerciseItemBase&{exerciseType:'matching';payload:{leftItems:ExerciseMatchingItem[];rightItems:ExerciseMatchingItem[]}}
  | GuidedExerciseItemBase&{exerciseType:'short_answer_exact';payload:{normalizationProfile:'english_basic_v1'}}
export interface GuidedExerciseSummary { exerciseCount:number;selectedCorrectCount:number;selectedIncorrectCount:number;totalAttemptCount:number;accuracyPercent:number }
export interface GuidedExerciseStage { engineVersion:number;stageSchemaVersion:number;attemptResultVersion:number;normalizationVersion:number;currentExerciseIndex:number;items:GuidedExerciseItem[];summary:GuidedExerciseSummary|null }
...
export type GuidedActiveContent = {kind:'theory';blocks:TheoryBlock[]}|{kind:'visual_vocabulary';items:VisualVocabularyItem[]}|{kind:'listening';segments:GuidedListeningSegment[];revealTextAfterFirstPlay:boolean}|{kind:'repeat';targets:GuidedRepeatTarget[]}|{kind:'speaking_check';targets:GuidedSpeakingTarget[]}|{kind:'exercise';stage:GuidedExerciseStage}
export interface GuidedActiveStage { stageId:string; sequenceIndex:number; stageType:InteractiveStageType; title:string; instructions:string; required:boolean; content:GuidedActiveContent }
export interface GuidedLessonSession { id:string; lessonId:string; contentVersion:number; title:string; cefrBand:CefrBand; status:InteractiveSessionStatus; currentStageIndex:number; stageCount:number; progressPercent:number; stages:GuidedSessionStage[]; activeStage:GuidedActiveStage|null; startedAt:string; completedAt:string|null; abandonedAt:string|null }
export interface GuidedLessonOverview { publishedLessonCount:number; activeSession:GuidedLessonSession|null; capabilities:GuidedStageCapability[] }
export interface GuidedAudio { playbackId:string;audioBase64:string;mimeType:'audio/wav';source:'bundled'|'piper'|'piper_cache';durationMs:number;runtimeVersion:number }
