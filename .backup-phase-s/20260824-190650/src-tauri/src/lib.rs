...
use interactive_lesson::{
    GuidedLessonOverviewDto, InteractiveLessonDetailDto, InteractiveLessonSessionDto,
    InteractiveLessonSummaryDto, StageActionRequest, StartInteractiveLessonRequest,
};
...
fn complete_guided_lesson_stage(
    ...
    state.guided_lessons.complete(request)
}
#[tauri::command]
fn skip_guided_lesson_stage(
    ...
    state.guided_lessons.skip(request)
}
#[tauri::command]
fn abandon_guided_lesson(
    ...
    state.guided_lessons.abandon(&session_id)
}
#[tauri::command]
...
async fn analyze_pronunciation(
    ...
    let result = result?;
    validate_result(&result, &request.target_text)?;
    state
        .pronunciation
        .save(&result, &request.source_type, request.source_id.as_deref())
}
