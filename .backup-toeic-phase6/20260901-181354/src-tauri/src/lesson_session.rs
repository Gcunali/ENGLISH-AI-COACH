        ...
        match event {
            ...
            VoiceEngineEvent::Transcript { text, message } => {
                ...
                let session = active
                    ...
                    .ok_or_else(|| {
                        "Voice event does not belong to the active lesson.".to_owned()
                    })?;
...
mod tests {
    ...
    #[test]
    fn terminal_and_metric_events_are_safe_after_lesson_completion() {
        let (directory, manager) = manager();
        let lesson = manager.begin_lesson(&metadata()).unwrap();
        manager.complete_lesson(&lesson.id).unwrap();
        let mut finished = VoiceEngineEvent::TeacherFinished {
            generation_id: Some("turn-1".to_owned()),
        };
        manager.enrich_event(&lesson.id, &mut finished).unwrap();
        let mut metric = VoiceEngineEvent::VoiceTurnMetrics {
            metrics: crate::voice_performance_repository::VoiceTurnPerformanceDto {
                ...
                last_voice_to_first_audio_ms: None,
                capture_end_to_first_audio_ms: None,
                tts_total_ms: None,
                teacher_playback_ms: None,
                teacher_turn_total_ms: None,
                tts_chunk_count: 0,
                cancelled: true,
                fallback_used: false,
        ...
        manager.enrich_event(&lesson.id, &mut metric).unwrap();
        std::fs::remove_dir_all(directory).unwrap();
    }
}
