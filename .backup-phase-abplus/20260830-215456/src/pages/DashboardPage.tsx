...
import { Activity, BookOpen, Clock3, MessageCircle, Mic2, PenLine, Play, ShieldCheck, Sparkles, Square, Star } from 'lucide-react'
...
import { PageHero, PageShell } from '../components/ProductUI'
...
export function DashboardPage() {
  ...
  const latestFinalTeacherMessage = [...messages].reverse().find((message) => message.role === 'teacher')?.text

  return <PageShell width="wide">
    <PageHero eyebrow="Good to see you" title="Practice and review your" accent="real progress." description="Your conversations, progress and next practice steps stay together here." />
    <WelcomePanel />
    ...
    {dashboard.data && dashboard.data.totalLessons > 0 && <section aria-label="Practice totals" className="dashboard-stat-grid">
      <FactCard icon={<BookOpen />} label="Total lessons" value={String(dashboard.data.totalLessons)} detail={`${dashboard.data.completedLessons} completed`} />
      <FactCard icon={<Clock3 />} label="Practice time" value={dashboard.data.totalPracticeSeconds === null ? 'Not available' : formatDuration(dashboard.data.totalPracticeSeconds)} detail="Completed + interrupted" />
      <FactCard icon={<MessageCircle />} label="Your turns" value={String(dashboard.data.totalStudentTurns)} detail="Persisted student turns" />
      <FactCard icon={<PenLine />} label="Corrections" value={String(dashboard.data.totalCorrections)} detail="Live correction cues" />
      <FactCard icon={<Star />} label="Average score" value={dashboard.data.averageOverallScore === null ? 'Not available' : String(dashboard.data.averageOverallScore)} detail={`${dashboard.data.analyzedLessons} analyzed`} />
    </section>}
    {memory.data && <section aria-label="Learning memory summary" className="dashboard-memory-grid"><FactCard icon={<BookOpen />} label="Vocabulary tracked" value={String(memory.data.vocabularyTotal)} detail={`${memory.data.vocabularyLearning} learning · ${memory.data.vocabularyKnown} known`} /><FactCard icon={<ShieldCheck />} label="Recurring mistakes" value={String(memory.data.recurringMistakesConfirmed)} detail="Confirmed across 2+ lessons" /></section>}
    ...
    {dashboard.data?.totalLessons === 0 && <div className="mb-5"><EmptyState title="Choose your first step" message="Start a private local conversation or take the Placement Test. Your real progress will appear here after you practice." action={<><Link to="/lesson/new" className="button-primary">Start a Conversation</Link><Link to="/placement" className="button-secondary">Take Placement Test</Link></>} /></div>}
    ...
    <div className="content-grid grid grid-cols-[minmax(0,1fr)_330px] gap-5">
      <section className="glass rounded-[28px] min-h-[560px] p-5 md:p-8 flex flex-col" aria-label="Conversation workspace">
        ...
        <div className="border-t border-white/[.07] pt-4">
          ...
          {showTranscript && <div className="mt-4 max-h-52 overflow-auto space-y-3" aria-label="Live transcript">
          ...
          <span className="sr-only" role="status" aria-live="polite">{latestFinalTeacherMessage ? `Teacher response complete: ${latestFinalTeacherMessage}` : ''}</span>
        </div>
      </section>
...
function FactCard({ label, value, detail, icon }: { label: string; value: string; detail?: string; icon?: React.ReactNode }) {
  return <div className="dashboard-fact-card">{icon && <span className="dashboard-fact-icon" aria-hidden="true">{icon}</span>}<div className="min-w-0"><div className="metric-label">{label}</div><div className="dashboard-fact-value">{value}</div>{detail && <div className="stat-card-detail">{detail}</div>}</div></div>
}
