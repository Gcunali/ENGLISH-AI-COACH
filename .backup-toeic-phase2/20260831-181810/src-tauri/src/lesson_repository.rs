use crate::{
    database,
    lesson_modes::{insert_snapshot, ValidatedLessonConfiguration},
};
...
impl LessonRepository {
    ...
    pub fn create_configured_lesson(
        &self,
        input: &NewLesson,
        ...
        let transaction = connection
            .transaction()
            .map_err(|error| format!("Could not start configured lesson transaction: {error}"))?;
        transaction
            .execute(
                &format!(
                    "INSERT INTO lesson (
          ...
          ollama_model, piper_voice, voice_engine_version, created_at, updated_at
            ...
            .map_err(|error| format!("Could not create configured lesson: {error}"))?;
        insert_snapshot(&transaction, &id, configuration)?;
        transaction
            .commit()
            .map_err(|error| format!("Could not commit configured lesson: {error}"))?;
        self.get_lesson(&id)?
            .ok_or_else(|| "Created configured lesson could not be read back.".to_owned())
    }
        ...
        connection
            .query_row(
                "SELECT id, started_at, ended_at, status, topic, mode, duration_seconds,
                 student_turn_count, teacher_turn_count, correction_count, whisper_model,
                 whisper_threads, ollama_model, piper_voice, voice_engine_version,
                 error_message, created_at, updated_at FROM lesson
                 WHERE status = 'completed' ORDER BY started_at DESC, created_at DESC LIMIT 1",
                [],
                row_to_lesson,
            )
            .optional()
            .map_err(|error| format!("Could not read latest completed lesson: {error}"))
    }
...
mod tests {
    ...
    #[test]
    fn builds_ordered_analysis_input_with_roles_and_correction_candidates() {
        let (directory, repository) = repository();
        let lesson = repository.create_lesson(&metadata()).unwrap();
        repository.mark_lesson_active(&lesson.id).unwrap();
        let first_student = repository
            .insert_student_message(&lesson.id, "Today I play tennis.")
            .unwrap()
            .unwrap();
        repository
            .insert_teacher_response(
            ...
            .unwrap();
        let second_student = repository
            .insert_student_message(&lesson.id, "Yes, I play with my friend yesterday.")
            .unwrap()
            .unwrap();
        repository
            .insert_teacher_response(
                &lesson.id,
                &second_student.id,
                "Small correction: say 'I played with my friend yesterday.'",
            )
            .unwrap();
        repository.complete_lesson(&lesson.id).unwrap();

        let input = repository.get_analysis_input(&lesson.id).unwrap();
        assert_eq!(
            input
                .transcript
                .iter()
        ...
        assert_eq!(input.correction_candidates.len(), 2);
        assert_eq!(
            input.correction_candidates[0].student_text,
            "Today I play tennis."
        );
        std::fs::remove_dir_all(directory).unwrap();
    }
