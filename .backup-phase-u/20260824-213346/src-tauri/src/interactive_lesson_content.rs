...
mod tests {
    ...
    #[test]
    fn phase_s_stages_are_available_and_future_executors_remain_closed() {
    ...
    fn invalid_order_and_unsupported_runtime_are_distinguished() {
        ...
        unavailable["lessonId"] = json!("future-guided-conversation");
        unavailable["stages"].as_array_mut().unwrap().truncate(1);
        unavailable["stages"].as_array_mut().unwrap().push(json!({"stageId":"guided","stageType":"guided_conversation","stageSchemaVersion":1,"title":"Guided Conversation","instructions":"Reserved stage.","required":false,"payload":{}}));
        write(&root, "unavailable", &unavailable);
        let registry = InteractiveLessonContentRegistry::load(root.clone());
        assert_eq!(registry.published_count(), 1);
        assert_eq!(registry.invalid_count(), 1);
        assert!(!summary(&registry.get("future-guided-conversation").unwrap()).startable);
        fs::remove_dir_all(root).unwrap();
    }
    #[test]
