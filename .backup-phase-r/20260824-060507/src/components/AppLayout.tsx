import { Activity, Award, BookOpen, CircleGauge, ClipboardCheck, History, Home, Mic2, RefreshCw, Settings, ShieldCheck, Sparkles, UserRound } from 'lucide-react'
...
function Navigation({ compact = false }: { compact?: boolean }) {
  return <nav className={compact ? 'scrollbar-none mt-3 flex gap-2 overflow-x-auto pb-1' : 'mt-5 text-sm'} aria-label="Main navigation">
    {NAVIGATION.map((group) => <div key={group.label} className={compact ? 'contents' : undefined}>{!compact && <div className="nav-group-label">{group.label}</div>}{group.items.map(({ to, label, icon: Icon, ...options }) => <NavLink
      key={to}
      to={to}
