  ...
  return <>
    ...
    <section className="glass rounded-[28px] p-5 md:p-7" aria-labelledby="voice-performance-title">
      ...
      <div className="flex flex-wrap items-start justify-between gap-5">
        ...
        <button type="button" role="switch" aria-label="Faster voice responses" aria-checked={streamingEnabled ?? false} disabled={streamingEnabled === null || savingStreaming} onClick={toggleStreaming} className={`min-w-24 shrink-0 rounded-full border px-4 py-2 text-sm ${streamingEnabled ? 'border-[var(--accent)] bg-[var(--accent)]/10 text-[var(--accent)]' : 'border-white/10 bg-white/[.03] text-white'} disabled:opacity-50`}>{streamingEnabled ? 'On' : 'Off'}</button>
      </div>
