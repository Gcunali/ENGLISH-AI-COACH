...
pub fn migrate(path: &Path) -> Result<(), String> {
    ...
    transaction
        .execute_batch(include_str!("../migrations/021_toeic_exam_center.sql"))
        .map_err(|error| format!("TOEIC Exam Center migration failed: {error}"))?;
    transaction
    ...
    transaction
        .execute_batch(include_str!(
            "../migrations/024_toeic_personalized_preparation.sql"
        ))
        .map_err(|error| format!("TOEIC personalized preparation migration failed: {error}"))?;
    transaction
        .commit()
...
mod tests {
    ...
    fn upgrades_version_eleven_to_pronunciation_without_fabricated_attempts() {
        ...
        let foreign_keys: i64 = connection
            ...
            .unwrap();
        assert_eq!((version, lessons, attempts), (24, 1, 0));
        assert_eq!(integrity, "ok");
        assert_eq!(foreign_keys, 0);
        drop(connection);
        std::fs::remove_dir_all(directory).unwrap();
