...
import { cancelCurrentTeacherResponse, endLesson as endPersistedLesson, getActiveLesson, getLatestCompletedLesson, getLesson, getLessonCorrections, getLessonTranscript, getVoiceEngineState, startLesson as startPersistedLesson, subscribeVoiceEngineEvents } from '../services/native'
import { useConversationStore } from '../stores/conversation'
import type { Lesson, LessonStartRequest, LessonSummary, VoiceEngineEvent, VoiceEngineState } from '../types'
...
export function useLocalVoiceEngine() {
  ...
  const startLesson = useCallback(async (request: LessonStartRequest) => {
    ...
    try {
      const result = await startPersistedLesson(request)
  ...
  const cancelTeacherResponse = useCallback(async () => {
    try {
      return await cancelCurrentTeacherResponse()
    } catch (error) {
  ...
  return { engineState, startLesson, endLesson, cancelTeacherResponse }
}
