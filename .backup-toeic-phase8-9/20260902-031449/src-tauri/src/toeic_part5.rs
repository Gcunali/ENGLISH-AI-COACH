...
mod tests {
    ...
    #[test]
    fn production_forms_are_complete_balanced_and_valid() {
        let b = Part5Bank::load(
            PathBuf::from(env!("CARGO_MANIFEST_DIR"))
                .join("resources/toeic/item-bank-v1/part5.json"),
        )
        .unwrap();
        assert_eq!(b.items.len(), 90);
        assert_eq!(b.forms.len(), 3);
        assert!(b.forms.values().all(|form| form.item_ids.len() == 30));
    }
    #[test]
    fn forms_b_and_c_start_with_their_frozen_items() {
        let (repo, db) = setup();
        for (family, first) in [("b", "0031"), ("c", "0061")] {
            let form_id = format!("toeic-part5-form-{family}");
            let session = repo.start(&form_id, 1, "learning").unwrap();
            assert_eq!(session.form_id, form_id);
            assert_eq!(
                session.current_question.as_ref().unwrap().item_id,
                format!("toeic-r-p5-{first}")
            );
        }
        drop(repo);
