...
mod tests {
    ...
    fn production_form_is_complete_and_balanced() {
        ...
        for (form_id, first_set, first_item) in [
            ("toeic-part7-form-a", "toeic-r-p7-set-s01", "toeic-r-p7-147"),
            (
                "toeic-part7-form-b",
                "toeic-r-p7-set-s01-b",
                "toeic-r-p7-b-001",
            ),
            (
                "toeic-part7-form-c",
                "toeic-r-p7-set-s01-c",
                "toeic-r-p7-c-001",
            ),
        ] {
            let s = r.start(form_id, 1, "learning").unwrap();
            let set = s.current_set.unwrap();
            assert_eq!(set.text_set_id, first_set);
            assert_eq!(set.questions.len(), 3);
            assert_eq!(set.questions[0].item_id, first_item);
        }
        drop(r);
