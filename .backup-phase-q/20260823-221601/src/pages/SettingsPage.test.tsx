...
const native = vi.hoisted(() => ({
  ...
  getStreamingVoiceResponseEnabled: vi.fn(),
  setStreamingVoiceResponseEnabled: vi.fn(),
  getBackupStatus: vi.fn(),
  listAppBackups: vi.fn(),
...
beforeEach(() => {
  ...
  native.getStreamingVoiceResponseEnabled.mockResolvedValue(true)
  native.setStreamingVoiceResponseEnabled.mockImplementation(async (enabled: boolean) => enabled)
  native.getBackupStatus.mockResolvedValue({ operation:'idle',error:null,backupDirectory:'C:\\backups',lastBackup:null,restoreAllowed:true,restoreBlockReason:null,pendingRestart:false,lastRestore:null })
...
describe('SettingsPage learning memory', () => {
  ...
  it('shows a persisted off setting', async () => {
    ...
    render(<SettingsPage />)
    const toggle = await screen.findByRole('switch', { name: 'Use learning memory in lessons' })
  ...
  it('shows persisted pedagogical memory and honest empty recurring state', async () => {
    ...
    expect(screen.getByRole('switch', { name: 'Use learning memory in lessons' })).toHaveAttribute('aria-checked', 'true')
  })
  ...
  it('persists Faster voice responses and rolls back a failed write', async () => {
    native.getStreamingVoiceResponseEnabled.mockResolvedValue(false)
    render(<SettingsPage />)
    const toggle = await screen.findByRole('switch', { name: 'Faster voice responses' })
    await waitFor(() => expect(toggle).toHaveAttribute('aria-checked', 'false'))
    fireEvent.click(toggle)
    await waitFor(() => expect(native.setStreamingVoiceResponseEnabled).toHaveBeenCalledWith(true))
    expect(toggle).toHaveAttribute('aria-checked', 'true')
    native.setStreamingVoiceResponseEnabled.mockRejectedValueOnce(new Error('voice setting failed'))
    fireEvent.click(toggle)
    expect(await screen.findByRole('alert')).toHaveTextContent('voice setting failed')
    expect(toggle).toHaveAttribute('aria-checked', 'true')
  })
})
