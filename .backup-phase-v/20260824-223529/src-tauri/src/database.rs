...
mod tests {
    ...
    fn upgrades_version_thirteen_to_fourteen_without_touching_existing_data() {
        ...
        connection.execute_batch("DROP TABLE interactive_lesson_stage_state; DROP TABLE interactive_lesson_session; DELETE FROM schema_migration WHERE version=14;").unwrap();
        drop(connection);
        migrate(&database).unwrap();
        let connection = Connection::open(&database).unwrap();
    ...
    fn upgrades_sixteen_to_seventeen_idempotently_without_fabricated_guided_turns() {
        ...
        migrate(&database).unwrap();
        let connection = Connection::open(&database).unwrap();
        connection.execute_batch("DROP TABLE interactive_lesson_guided_conversation_turn; DELETE FROM schema_migration WHERE version=17;").unwrap();
        drop(connection);
        migrate(&database).unwrap();
        migrate(&database).unwrap();
        let connection = Connection::open(&database).unwrap();
    ...
    fn upgrades_version_eleven_to_pronunciation_without_fabricated_attempts() {
        ...
        let foreign_keys: i64 = connection
            ...
            .unwrap();
        assert_eq!((version, lessons, attempts), (17, 1, 0));
        assert_eq!(integrity, "ok");
        assert_eq!(foreign_keys, 0);
        drop(connection);
        std::fs::remove_dir_all(directory).unwrap();
    ...
    fn physical_phase_u_migrates_without_fabricating_guided_conversation_data() {
        ...
        println!("physical_phase_u|schema=17|counts={after_counts:?}|guided_turns=0|integrity=ok|foreign_keys=0");
    }
}
