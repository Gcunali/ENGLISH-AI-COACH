...
mod tests {
    ...
    #[test]
    fn upgrades_fifteen_to_sixteen_idempotently_without_fabricated_exercise_attempts() {
        let (directory, database) = test_database();
        migrate(&database).unwrap();
        let connection = Connection::open(&database).unwrap();
        connection.execute_batch("DROP TABLE interactive_lesson_exercise_attempt; DELETE FROM schema_migration WHERE version=16;").unwrap();
        drop(connection);
        migrate(&database).unwrap();
        migrate(&database).unwrap();
        let connection = Connection::open(&database).unwrap();
    ...
    fn upgrades_version_eleven_to_pronunciation_without_fabricated_attempts() {
        ...
        let foreign_keys: i64 = connection
            ...
            .unwrap();
        assert_eq!((version, lessons, attempts), (16, 1, 0));
        assert_eq!(integrity, "ok");
        assert_eq!(foreign_keys, 0);
        drop(connection);
        std::fs::remove_dir_all(directory).unwrap();
