...
import { getLessonModes } from '../services/native'
...
export function NewLessonPage() {
  ...
  const load = () => {
    ...
    void getLessonModes().then(setModes).catch((reason: unknown) => setError(reason instanceof Error ? reason.message : String(reason)))
  }
  useEffect(load, [])
  const selected = useMemo(() => modes?.find((item) => item.id === selectedId) ?? null, [modes, selectedId])

  function choose(mode: LessonModeDefinition) {
    setSelectedId(mode.id); setDifficulty(mode.defaultDifficulty); setFocusAreas([])
