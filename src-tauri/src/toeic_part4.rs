...
mod tests {
    ...
    fn setup() -> (Part4Repository, PathBuf) {
        ...
        let bank=Part4Bank::load(PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("resources/toeic/item-bank-v1/part4.json")).unwrap();
    ...
    fn hear(repo:&Part4Repository,s:&Session) {
        ...
        repo.finish_audio(&s.session_id,&set.set_id,p.presentation_id.as_deref(),true).unwrap();
    }
    #[test]
    fn production_form_is_complete_and_valid() {
        let b=Part4Bank::load(PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("resources/toeic/item-bank-v1/part4.json")).unwrap();
        assert_eq!(b.sets.len(),10);
        assert_eq!(b.forms["toeic-part4-form-a"].set_ids.len(),10);
        assert_eq!(b.sets.values().map(|s|s.questions.len()).sum::<usize>(),30);
        assert!(b.sets.values().all(|s|s.speakers.len()==1&&s.turns.len()==1));
    }
    #[test]
    fn public_payload_hides_script_keys_and_feedback_until_third_answer() {
        ...
        s=repo.submit(Submit{session_id:s.session_id.clone(),question_id:q,selected_choice:"A".into()}).unwrap();
    ...
    fn completes_exactly_thirty_and_resumes() {
        ...
        while s.status=="in_progress" {
            hear(&repo,&s);
            let ids=s.current_set.as_ref().unwrap().questions.iter().map(|q|q.question_id.clone()).collect::<Vec<_>>();
            for id in ids { s=repo.submit(Submit{session_id:s.session_id.clone(),question_id:id,selected_choice:"A".into()}).unwrap(); }
            if s.status=="in_progress" { s=repo.advance(&s.session_id).unwrap(); }
        }
        let result=repo.result(&s.session_id).unwrap();
        assert_eq!(result.total,30);assert_eq!(repo.review(&s.session_id,false).unwrap().len(),10);
        assert_eq!(repo.session(&s.session_id).unwrap().status,"completed");
        drop(repo);let _=std::fs::remove_file(db);
    }
    #[test]
    fn interrupted_audio_can_restart_but_completed_audio_cannot() {
        let(repo,db)=setup();let s=repo.start("toeic-part4-form-a",1).unwrap();
        let p=repo.begin_audio(&s.session_id,0,None).unwrap();repo.interrupt(p.presentation_id.as_deref()).unwrap();
