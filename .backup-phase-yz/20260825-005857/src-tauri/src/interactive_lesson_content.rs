...
mod tests {
    ...
    #[test]
    fn production_phase_x_all_packages_are_published_and_valid() {
        let root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("resources")
            .join("interactive-lessons");
        let registry = InteractiveLessonContentRegistry::load(root);
        ...
        assert_eq!(registry.exact_versions.len(), 96);
        let mut a1 = 0;
        let mut a2 = 0;
        for registered in registry.exact_versions.values() {
            ...
            match package.cefr_band {
                crate::placement::CefrBand::A1 => a1 += 1,
                crate::placement::CefrBand::A2 => a2 += 1,
                _ => panic!("Phase X production content must contain only A1/A2"),
            }
        }
        assert_eq!((a1, a2), (48, 48));
    }
}
