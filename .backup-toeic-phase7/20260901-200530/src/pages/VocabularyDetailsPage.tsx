import { ArrowLeft, BookOpen, Mic2 } from 'lucide-react'
...
export function VocabularyDetailsPage() {
  ...
  return <PageShell width="standard">
    ...
    <header className="glass rounded-[28px] p-5 md:p-7"><div className="flex flex-wrap items-start justify-between gap-5"><div className="flex items-start gap-3"><BookOpen className="mt-1 text-[var(--accent)]" size={20} /><div><p className="muted mb-1 text-[10px] uppercase tracking-widest">Vocabulary item</p><h1 className="m-0 text-2xl md:text-3xl">{item.text}</h1><p className="muted mb-0 mt-3 text-sm">{item.meaning}</p><Link className="button-secondary mt-4" to={`/pronunciation?text=${encodeURIComponent(item.text)}&source=vocabulary&sourceId=${encodeURIComponent(item.id)}`}><Mic2 size={16}/> Practice pronunciation</Link></div></div><label className="text-xs">Learning status <select aria-label="Learning status" value={status} disabled={saving} onChange={(event) => void changeStatus(event.target.value as VocabularyStatus)} className="ml-2 rounded-xl border border-white/10 bg-[#0d1119] px-3 py-2 text-white"><option value="new">New</option><option value="learning">Learning</option><option value="known">Known</option></select></label></div>{saveError && <p role="alert" className="mt-4 text-sm text-red-200">{saveError}</p>}<div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Detail label="Status" value={humanize(status)} /><Detail label="First seen" value={formatLocalDate(item.firstSeenAt)} /><Detail label="Last seen" value={formatLocalDate(item.lastSeenAt)} /><Detail label="Lessons" value={String(item.lessonCount)} /></div></header>
    <section className="glass mt-5 rounded-[28px] p-5 md:p-7"><h2 className="mt-0 text-lg">Examples from completed lessons</h2><div className="space-y-3">{occurrences.map((occurrence, index) => <article key={`${occurrence.sourceType}-${occurrence.lessonId}-${index}`} className="rounded-2xl border border-white/[.07] bg-white/[.025] p-4"><div className="flex flex-wrap items-center justify-between gap-3"><span className="muted text-xs">{formatLocalDate(occurrence.lessonDate)} · {occurrence.sourceType === 'guided' ? 'Guided Lesson' : 'Conversation lesson'}</span><Link to={occurrence.sourceType === 'guided' ? `/guided-lessons/${occurrence.lessonId}` : `/history/${occurrence.lessonId}`} className="text-xs text-white">View lesson</Link></div><p className="mb-0 mt-3 text-sm">“{occurrence.example}”</p>{occurrence.occurrenceCount > 1 && <p className="muted mb-0 mt-2 text-[10px]">{occurrence.occurrenceCount} structured occurrences in this lesson</p>}</article>)}</div></section>
  </PageShell>
}
