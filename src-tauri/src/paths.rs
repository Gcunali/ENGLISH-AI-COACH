...
pub struct LocalPaths {
    ...
    pub static_tts_cache: PathBuf,
    pub interactive_lesson_assets: PathBuf,
...
impl LocalPaths {
    pub fn create(root: PathBuf) -> Result<Self, String> {
        let paths = Self {
            ...
            static_tts_cache: root.join("cache/static_tts"),
            interactive_lesson_assets: root.join("interactive_lesson_assets"),
            database: root.join("database"),
            backups: root.join("backups"),
            reliability: root.join("reliability"),
            logs: root.join("logs"),
        ...
        for directory in [
            ...
            &paths.static_tts_cache,
            &paths.interactive_lesson_assets,
            &paths.database,
            &paths.backups,
            &paths.reliability,
            &paths.logs,
...
impl LocalAiPaths {
    ...
    pub fn pronunciation_root(&self) -> PathBuf {
        self.local_ai_root.join("pronunciation")
    }

    pub fn pronunciation_python(&self) -> PathBuf {
        self.pronunciation_root().join(".venv/Scripts/python.exe")
    }

    pub fn pronunciation_worker(&self) -> PathBuf {
        self.pronunciation_root().join("pronunciation_engine.py")
    }

    pub fn pronunciation_model_dir(&self) -> PathBuf {
        self.pronunciation_root()
            .join("models/wav2vec2-lv-60-espeak-cv-ft")
    }
...
mod tests {
    ...
    fn constructs_local_ai_paths_from_an_injected_project_root() {
        ...
        assert_eq!(
            paths.pronunciation_worker(),
            root.join("local-ai/pronunciation/pronunciation_engine.py")
        );
    }
}
