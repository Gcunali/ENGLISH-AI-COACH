...
describe('DataBackupSection',()=>{
  ...
  it('requires explicit confirmation before scheduling restore',async()=>{render(<MemoryRouter><DataBackupSection/></MemoryRouter>);const restore=await screen.findByRole('button',{name:/Restore backup/});fireEvent.click(restore);expect(screen.getByRole('dialog')).toHaveTextContent('A safety backup of your current data will be created first.');expect(native.restoreAppBackup).not.toHaveBeenCalled();fireEvent.click(screen.getByRole('button',{name:'Restore Backup'}));await waitFor(()=>expect(native.restoreAppBackup).toHaveBeenCalledWith(backup.backupId))})
