...
import { InteractiveLessonAnalysisStage } from "../components/InteractiveLessonAnalysisStage";
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      ...
      const eventId = globalThis.crypto?.randomUUID?.() ??
        `${session.id}-${Date.now()}-${Math.random().toString(16).slice(2)}`;
      void recordGuidedPracticeTime(session.id, eventId, 15).catch(() => undefined);
    }, 15_000);
    ...
    return (
      <PageShell width="standard">
        <section className="glass rounded-[28px] p-6 md:p-8">
          ...
          {session.stages.find((item) => item.stageType === "analysis") ? (
            <InteractiveLessonAnalysisStage
              session={session}
              stageId={session.stages.find((item) => item.stageType === "analysis")!.stageId}
              updateSession={setSession}
            />
          ) : (
            <p className="page-description text-center">This lesson did not include a review stage.</p>
          )}
          <div className="mt-6 text-center"><Link className="button-primary inline-flex no-underline" to="/guided-lessons">Back to Guided Lessons</Link></div>
        </section>
      </PageShell>
    );
...
function StageContent(props: {
  ...
  if (content.kind === "analysis")
    return (
      <InteractiveLessonAnalysisStage
        session={props.session}
        stageId={props.session.activeStage!.stageId}
        updateSession={props.update}
      />
    );
