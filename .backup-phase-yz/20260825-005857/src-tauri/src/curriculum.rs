...
pub const CURRICULUM_RECOMMENDATION_VERSION: u32 = 1;
...
mod tests {
    ...
    fn no_placement_never_fabricates_a1_and_safe_dto_has_no_private_content() {
        let (root, service) = temp_service();
        let json = serde_json::to_string(&service.catalog().unwrap()).unwrap();
    ...
    fn production_phase_x_english_core_resolves_16_units_96_lessons_at_zero_progress() {
        ...
        assert_eq!(course.progress.total_lessons, 96);
        assert_eq!(course.progress.completed_lessons, 0);
        assert_eq!(course.progress.percent, 0);
        assert!(course
            .levels
        ...
        assert_eq!(course.suggested_level, Some(CefrBand::B1));
        assert!(course.levels.iter().all(|level| !level.recommended));
        fs::remove_dir_all(root).unwrap();
    }
