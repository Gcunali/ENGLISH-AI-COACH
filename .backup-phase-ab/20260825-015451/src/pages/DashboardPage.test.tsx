...
vi.mock('../hooks/useGamification', () => ({ useGamificationOverview: () => ({ data: { totalXp: 48, practiceLevel: 1, currentStreakDays: 1, totalPracticeMinutes: 2, weeklyGoal: { practicedMinutes: 2, goalMinutes: 90, progressPercent: 2, reached: false }, unlockedAchievementCount: 1, totalAchievementCount: 11 }, loading: false, error: null, reload }) }))
vi.mock('../components/ReviewOverviewCard', () => ({ ReviewOverviewCard: () => <section aria-label="Suggested Review">Review block</section> }))
vi.mock('../services/native', () => ({ probeLocalVoiceEngine: mocks.probeLocalVoiceEngine, getCourseCatalog: mocks.getCourseCatalog, getWelcomeState: mocks.getWelcomeState, setWelcomeSeen: mocks.setWelcomeSeen }))
...
describe('DashboardPage', () => {
  beforeEach(() => {
    ...
    mocks.useLocalVoiceEngine.mockReturnValue({ engineState: 'stopped', startLesson: vi.fn(), endLesson: vi.fn(), cancelTeacherResponse: vi.fn() })
    ...
    mocks.getCourseCatalog.mockResolvedValue({ curricula: [] })
    mocks.getWelcomeState.mockResolvedValue({ shouldShow: false, hasSeen: true, existingUser: true })
    mocks.setWelcomeSeen.mockResolvedValue({ shouldShow: false, hasSeen: true, existingUser: true })
  ...
  it('renders the factual empty state while keeping Start Lesson', () => {
    ...
    render(<MemoryRouter><DashboardPage /></MemoryRouter>)
    expect(screen.getByText('Choose your first step')).toBeInTheDocument()
  ...
  it('renders persisted totals, latest scores and recommendation', () => {
    ...
    expect(screen.getByLabelText('Suggested Review')).toBeInTheDocument()
  })

  it('shows the streamed draft and an accessible Stop response control', () => {
    const cancelTeacherResponse = vi.fn(async () => true)
    mocks.useLocalVoiceEngine.mockReturnValue({ engineState: 'running', startLesson: vi.fn(), endLesson: vi.fn(), cancelTeacherResponse })
    mocks.useDashboardData.mockReturnValue({ data: dashboard, loading: false, error: null, reload })
    const store = useConversationStore.getState()
    store.applyVoiceEngineEvent({ type: 'teacher_thinking' })
    store.applyVoiceEngineEvent({ type: 'teacher_stream_started', generationId: 'turn-1' })
    store.applyVoiceEngineEvent({ type: 'teacher_response_delta', generationId: 'turn-1', delta: 'Hello', text: 'Hello there.' })
    render(<MemoryRouter><DashboardPage /></MemoryRouter>)
    expect(screen.getByTestId('teacher-draft')).toHaveTextContent('Hello there.')
    fireEvent.click(screen.getByRole('button', { name: 'Stop teacher response' }))
    expect(cancelTeacherResponse).toHaveBeenCalledOnce()
    expect(screen.getByRole('button', { name: 'End Lesson' })).toBeInTheDocument()
  })
})
