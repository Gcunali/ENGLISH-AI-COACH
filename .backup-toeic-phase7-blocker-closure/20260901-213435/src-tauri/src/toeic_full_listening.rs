...
impl FullListeningRepository {
    ...
    pub fn start(&self, mode: &str) -> Result<FullSession, String> {
        ...
        for (part, sid, form) in [
            (1, p1.session_id, "toeic-listening-form-a"),
            (2, p2.session_id, "toeic-part2-form-a"),
            (3, p3.session_id, "toeic-part3-form-a"),
            (4, p4.session_id, "toeic-part4-form-a"),
        ] {
    ...
    pub fn session(&self, id: &str) -> Result<FullSession, String> {
        let c = database::open(&self.database)?;
        let (mode, parent_status) = c
            .query_row(
                "SELECT mode,status FROM toeic_full_listening_session WHERE id=?1",
                [id],
