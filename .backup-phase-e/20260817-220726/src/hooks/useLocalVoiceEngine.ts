...
import { endLesson as endPersistedLesson, getActiveLesson, getLesson, getLessonCorrections, getLessonTranscript, getVoiceEngineState, startLesson as startPersistedLesson, subscribeVoiceEngineEvents } from '../services/native'
...
export function useLocalVoiceEngine() {
  ...
  useEffect(() => {
    ...
    void getActiveLesson().then(async (lesson) => {
      if (!disposed && lesson) {
        const [messages, corrections] = await Promise.all([
          getLessonTranscript(lesson.id),
          getLessonCorrections(lesson.id),
        ])
        if (!disposed) useConversationStore.getState().restoreLesson(lesson, messages, corrections)
      }
    })
    return () => {
      disposed = true
