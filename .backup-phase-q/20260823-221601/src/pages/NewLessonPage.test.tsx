...
const mocks = vi.hoisted(() => ({ getLessonModes: vi.fn(), getStudentLearningProfile: vi.fn(), startLesson: vi.fn() }))
vi.mock('../services/native', () => ({ getLessonModes: mocks.getLessonModes, getStudentLearningProfile: mocks.getStudentLearningProfile }))
...
beforeEach(() => { mocks.getLessonModes.mockResolvedValue(modes); mocks.getStudentLearningProfile.mockResolvedValue({ schemaVersion: 1, currentPlacement: null, targetLevel: null, learningGoals: [], defaultLessonDifficulty: 'standard', useProfileInLessons: true }); mocks.startLesson.mockResolvedValue(true) })
...
describe('NewLessonPage', () => {
  ...
  it('uses the profile default initially while preserving the lesson override', async () => {
    mocks.getStudentLearningProfile.mockResolvedValue({ schemaVersion: 1, currentPlacement: null, targetLevel: null, learningGoals: [], defaultLessonDifficulty: 'challenging', useProfileInLessons: true })
    page(); fireEvent.click(await screen.findByRole('button', { name: /Travel English/ }))
    expect(screen.getByRole('button', { name: 'Challenging' })).toHaveAttribute('aria-pressed', 'true')
    fireEvent.click(screen.getByRole('button', { name: 'Easy' }))
    fireEvent.click(screen.getByRole('button', { name: 'Start Lesson' }))
    await waitFor(() => expect(mocks.startLesson).toHaveBeenCalledWith({ modeId: 'travel_english', difficulty: 'easy', focusAreas: [] }))
  })
})
