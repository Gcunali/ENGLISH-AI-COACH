...
impl Part6Bank {
    pub fn load(path: PathBuf) -> Result<Self, String> {
        let raw: Raw = serde_json::from_slice(
            &fs::read(path).map_err(|e| format!("Part 6 bank missing: {e}"))?,
        )
        .map_err(|e| format!("Part 6 bank invalid: {e}"))?;
        validate(&raw)?;
        Ok(Self {
            bank_id: raw.bank_id,
            forms: raw
...
fn validate(raw: &Raw) -> Result<(), String> {
    if raw.schema_version != 1 {
        return Err("Unsupported Part 6 schema.".into());
    }
    ...
    let skills = BTreeSet::from(["grammar", "vocabulary", "cohesion", "sentence_insertion"]);
    ...
    for set in &raw.text_sets {
        if !set_ids.insert(set.text_set_id.clone())
            || !set.text_set_id.starts_with("toeic-r-p6-set-")
            || set.version != 1
            || set.publication_state != "published"
            || !docs.contains(set.document_type.as_str())
            || !diffs.contains(set.difficulty.as_str())
            || set.passage.len() < 450
            || set.questions.len() != 4
        {
            return Err(format!("Invalid Part 6 text set {}.", set.text_set_id));
        }
        let mut insertion = 0;
        for q in &set.questions {
            ...
            if !item_ids.insert(q.item_id.clone())
                || !q.item_id.starts_with("toeic-r-p6-")
                || q.item_version != 1
                || q.publication_state != "published"
                || !types.contains(q.question_type.as_str())
                || !skills.contains(q.skill_category.as_str())
                || !diffs.contains(q.difficulty.as_str())
                || set.passage.matches(&format!("[{}]", q.blank_id)).count() != 1
                || q.choices.len() != 4
            ...
            {
                return Err(format!("Invalid Part 6 question {}.", q.item_id));
            }
            if q.question_type == "sentence_insertion" {
                insertion += 1;
                if q.choices
                    .iter()
                    .any(|x| !x.text.trim_end().ends_with(['.', '!', '?']))
                {
                    return Err(format!(
                        "Sentence insertion {} has an incomplete choice.",
                        q.item_id
                    ));
                }
            }
        ...
        if insertion != 1 {
            return Err(format!(
                "{} must have one sentence insertion.",
                set.text_set_id
            ));
        }
    }
    for form in &raw.forms {
        if form.form_version != 1
            ...
            || form.text_set_ids.len() != 4
            || form.text_set_ids.iter().collect::<BTreeSet<_>>().len() != 4
            || form.text_set_ids.iter().any(|x| !set_ids.contains(x))
        {
            return Err(format!("{} must contain four unique sets.", form.form_id));
        }
        let qs = form
            .text_set_ids
            .iter()
        ...
        for q in qs {
            ...
            *categories
                .entry(q.skill_category.as_str())
                .or_insert(0usize) += 1
        }
        if answers.values().any(|n| *n < 3 || *n > 5)
            || categories.get("sentence_insertion") != Some(&4)
        {
            return Err(format!(
                "Suspicious Part 6 distribution in {}.",
                form.form_id
            ));
        }
    }
...
impl Part6Repository {
    ...
    pub fn start(&self, form_id: &str, version: u32, mode: &str) -> Result<Session, String> {
        if !matches!(mode, "learning" | "simulation") {
            return Err("Part 6 mode must be learning or simulation.".into());
        }
        let c = database::open(&self.database)?;
        ...
        c.execute("INSERT INTO toeic_session(id,form_id,form_version,section,part,status,schema_version,form_snapshot_json,current_question_index,created_at,updated_at) VALUES(?1,?2,?3,'reading','part6_text_completion','in_progress',1,?4,0,strftime('%Y-%m-%dT%H:%M:%fZ','now'),strftime('%Y-%m-%dT%H:%M:%fZ','now'))",params![id,form_id,version,serde_json::to_string(&snap).unwrap()]).map_err(err)?;
        self.session(&id)
    }
    pub fn session(&self, id: &str) -> Result<Session, String> {
        let c = database::open(&self.database)?;
        let(status,json,index,created,updated)=c.query_row("SELECT status,form_snapshot_json,current_question_index,created_at,updated_at FROM toeic_session WHERE id=?1 AND part='part6_text_completion'",[id],|r|Ok((r.get::<_,String>(0)?,r.get::<_,String>(1)?,r.get::<_,u32>(2)?,r.get::<_,String>(3)?,r.get::<_,String>(4)?))).optional().map_err(err)?.ok_or("Part 6 session not found.")?;
        let snap: Snapshot = serde_json::from_str(&json).map_err(|_| "Invalid Part 6 snapshot.")?;
    ...
    pub fn submit(&self, r: Submit) -> Result<Session, String> {
        ...
        if s.status != "in_progress" {
            return Err("Part 6 session is complete.".into());
        }
        if s.set_feedback.is_some() {
            return Err("Continue to the next text before answering.".into());
        }
        let set = s.current_set.ok_or("No current Part 6 text.")?;
        let q = set
            .questions
            .iter()
            .find(|q| !q.locked)
        ...
        if q.item_id != r.item_id || q.item_version != r.item_version {
            return Err("Stale Part 6 question.".into());
        }
        let c = database::open(&self.database)?;
        let json: String = c
            .query_row(
        ...
        if n != 1 {
            return Err("The first Part 6 answer is final.".into());
        }
        c.execute(
    ...
    pub fn advance(&self, id: &str) -> Result<Session, String> {
        ...
        let set = s.current_set.as_ref().ok_or("No current Part 6 text.")?;
        if !set.questions.iter().all(|q| q.locked) {
            return Err("Answer all four questions before continuing.".into());
        }
        let c = database::open(&self.database)?;
        if s.current_set_index == 3 {
            c.execute("UPDATE toeic_session SET status='completed',current_question_index=4,completed_at=strftime('%Y-%m-%dT%H:%M:%fZ','now'),updated_at=strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE id=?1",[id]).map_err(err)?;
        } else {
    ...
    pub fn result(&self, id: &str) -> Result<ResultDto, String> {
        ...
        let needs_attention = skill_breakdown
            ...
            .map(|x| x.label.clone())
            .collect();
        Ok(ResultDto{session_id:id.into(),form_id:snap.form_id,correct,total:16,accuracy:correct*100/16,skill_breakdown,difficulty_breakdown:breakdown(diffs),document_breakdown:breakdown(docs),needs_attention,has_scaled_score:false,score_message:"Parts 5 and 6 provide 46/100 Reading questions. Part 7 is not implemented, so no Reading scaled score is estimated.".into()})
    }
    pub fn review(&self, id: &str, mistakes: bool) -> Result<Vec<ReviewSet>, String> {
        ...
        Ok(snap
            ...
            .map(|(i, s)| {
                let mut feedback = set_feedback(s, &ans, i as u32);
                if mistakes {
                    feedback.questions.retain(|q| !q.is_correct);
                }
                ReviewSet {
    ...
    fn completed(&self, id: &str) -> Result<(Snapshot, BTreeMap<String, (String, bool)>), String> {
        let c = database::open(&self.database)?;
        let(status,json)=c.query_row("SELECT status,form_snapshot_json FROM toeic_session WHERE id=?1 AND part='part6_text_completion'",[id],|r|Ok((r.get::<_,String>(0)?,r.get::<_,String>(1)?))).map_err(err)?;
        if status != "completed" {
            return Err("Complete Part 6 before viewing results.".into());
        }
        Ok((
            serde_json::from_str(&json).map_err(|_| "Invalid Part 6 snapshot.")?,
            answers(&c, id)?,
        ))
    }
}
    ...
    PublicSet {
        ...
        questions: s
            ...
            .map(|(i, q)| PublicQuestion {
                ...
                question_number: 131 + index * 4 + i as u32,
...
fn set_feedback(s: &TextSet, ans: &BTreeMap<String, (String, bool)>, index: u32) -> SetFeedback {
    let mut completed = s.passage.clone();
    for q in &s.questions {
        let correct = &q
            .choices
            .iter()
            .find(|c| c.choice == q.correct_answer)
            .unwrap()
            .text;
        completed = completed.replace(&format!("[{}]", q.blank_id), correct)
    }
    SetFeedback {
        text_set_id: s.text_set_id.clone(),
...
fn err(e: rusqlite::Error) -> String {
    format!("Part 6 database error: {e}")
}

#[cfg(test)]
mod tests {
    use super::*;
    fn setup() -> (Part6Repository, PathBuf) {
        let db = std::env::temp_dir().join(format!("toeic-p6-{}.db", uuid::Uuid::new_v4()));
        database::migrate(&db).unwrap();
        let bank = Part6Bank::load(
            PathBuf::from(env!("CARGO_MANIFEST_DIR"))
                .join("resources/toeic/item-bank-v1/part6.json"),
        )
        .unwrap();
        (Part6Repository::new(db.clone(), bank), db)
    }
    #[test]
    fn production_form_is_complete_and_balanced() {
        let (r, db) = setup();
        let o = r.overview().unwrap();
        assert_eq!(o.forms.len(), 3);
        for family in ["b", "c"] {
            let form_id = format!("toeic-part6-form-{family}");
            let s = r.start(&form_id, 1, "learning").unwrap();
            assert_eq!(s.form_id, form_id);
            assert_eq!(
                s.current_set.as_ref().unwrap().text_set_id,
                format!("toeic-r-p6-set-{family}01")
            );
            assert_eq!(s.current_set.unwrap().questions.len(), 4);
        }
    ...
    fn feedback_does_not_leak_before_fourth_answer() {
        let (r, db) = setup();
        let s = r.start("toeic-part6-form-a", 1, "learning").unwrap();
        for n in 0..3 {
    ...
    fn first_attempt_is_immutable_and_set_feedback_unlocks() {
        let (r, db) = setup();
        let mut s = r.start("toeic-part6-form-a", 1, "learning").unwrap();
        for _ in 0..4 {
        ...
        assert_eq!(s.set_feedback.as_ref().unwrap().questions.len(), 4);
    ...
    fn simulation_hides_feedback_and_completes_sixteen() {
        let (r, db) = setup();
        let mut s = r.start("toeic-part6-form-a", 1, "simulation").unwrap();
        ...
        assert_eq!(r.review(&s.session_id, false).unwrap().len(), 4);
        let _ = fs::remove_file(db);
    }
}
